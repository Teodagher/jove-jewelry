        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          {jewelryItems.map((item, index) => (
            <Link key={item.name} href={item.link} className="group">
              <div className="relative overflow-hidden bg-zinc-700 border border-zinc-600 hover:border-amber-300 transition-all duration-300 group-hover:shadow-xl">
                {/* Subtle accent line */}
                ...
                {/* Image Container */}
                <div className="relative h-80 bg-zinc-600 overflow-hidden">
                  <Image
                    src={item.image}
                    alt={item.name}
                    fill
                ...
                {/* Content */}
                <div className="p-8">
                  <h3 className="text-2xl font-light mb-4 tracking-wide text-white group-hover:text-amber-100 transition-colors">
                    {item.name}
                  </h3>
                  <p className="text-gray-300 leading-relaxed mb-6">
                    {item.description}
                  </p>
                  <div className="flex items-center text-gray-200 font-medium group-hover:translate-x-1 transition-transform duration-300">
