...
export default function ProductImages({ productId, productType, productSlug, refreshTrigger }: ProductImagesProps) {
  ...
  const handleFileSelect = async (variantId: string, file: File) => {
    ...
    try {
      ...
      // Compress image with reduced compression (~200-300KB target) using browser-image-compression
      const options = {
        maxSizeMB: 0.25, // 250KB target for better quality (reduced compression)
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-gray-600 text-sm">
            Upload images for each product variant. Images will be automatically compressed to WebP format (~250KB).
          </p>
        </div>
        <button
