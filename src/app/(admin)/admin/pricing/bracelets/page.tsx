...
export default function BraceletsPricingPage() {
  ...
  const updateBasePriceGold = async (newPrice: number) => {
    try {
      setSaving(true);
      const success = await CustomizationService.updateBasePriceGold('bracelet', newPrice);
  ...
  const updateBasePriceSilver = async (newPrice: number) => {
    try {
      setSaving(true);
      const success = await CustomizationService.updateBasePriceSilver('bracelet', newPrice);
  ...
  const updateOptionPriceGold = async (settingId: string, optionId: string, newPrice: number) => {
    try {
      setSaving(true);
      const success = await CustomizationService.updateOptionPriceGold('bracelet', settingId, optionId, newPrice);
  ...
  const updateOptionPriceSilver = async (settingId: string, optionId: string, newPrice: number) => {
    try {
      setSaving(true);
      const success = await CustomizationService.updateOptionPriceSilver('bracelet', settingId, optionId, newPrice);
  ...
  const getCurrentBlackOnyxPrice = () => {
    if (!pricingData) return 0;
    return selectedDiamondType === 'natural' 
      ? (pricingData.black_onyx_base_price || 0) 
      : (pricingData.black_onyx_base_price_lab_grown || 0);
  };
      ...
      {pricingData ? (
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <div className="space-y-8">
            ...
            {/* Black Onyx Base Price - Only show in Diamond Type mode */}
            {pricingMode === 'diamond_type' && (
              <div className="border-b pb-6">
                <h2 className="text-xl font-semibold text-gray-900 mb-4">
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <input
                    type="number"
                    step="0.01"
                    key={`black-onyx-${selectedDiamondType}`}
                    defaultValue={getCurrentBlackOnyxPrice()}
                    className="border border-gray-300 rounded px-3 py-2 w-32"
                    onBlur={(e) => {
                      const newPrice = parseFloat(e.target.value);
                      const currentPrice = getCurrentBlackOnyxPrice();
                      if (newPrice !== currentPrice && newPrice >= 0) {
                        if (selectedDiamondType === 'natural') {
