...
export default function EarringsPricingPage() {
  ...
  const updateBasePrice = async (newPrice: number) => {
    try {
      setSaving(true);
      const success = await CustomizationService.updateBasePrice('earrings', newPrice);
  ...
  const updateBasePriceLabGrown = async (newPrice: number) => {
    try {
      setSaving(true);
      const success = await CustomizationService.updateBasePriceLabGrown('earrings', newPrice);
  ...
  const updateOptionPrice = async (settingId: string, optionId: string, newPrice: number) => {
    try {
      setSaving(true);
      const success = await CustomizationService.updateOptionPrice('earrings', settingId, optionId, newPrice);
  ...
  const updateOptionPriceLabGrown = async (settingId: string, optionId: string, newPrice: number) => {
    try {
      setSaving(true);
      const success = await CustomizationService.updateOptionPriceLabGrown('earrings', settingId, optionId, newPrice);
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="border-b border-gray-200 pb-5">
        <h1 className="text-3xl font-light text-gray-900 font-serif tracking-wider">Earrings Pricing</h1>
      ...
      {pricingData ? (
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <div className="text-center text-gray-500">
            <p>No earring pricing data available. Set up your earring customization options first.</p>
          </div>
        </div>
      )}
